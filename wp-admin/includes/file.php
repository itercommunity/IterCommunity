...
function _unzip_file_ziparchive($file, $to, $needed_dirs = array() ) {
	...
	foreach ( $needed_dirs as $_dir ) {
		if ( ! $wp_filesystem->mkdir($_dir) && ! $wp_filesystem->is_dir($_dir)) // Only check to see if the Dir exists upon creation failure. Less I/O this way.
			return new WP_Error( 'mkdir_failed_ziparchive', __( 'Could not create directory(636). '), substr( $_dir, strlen( $to ) ) );
	}
	unset($needed_dirs);
...
function _unzip_file_pclzip($file, $to, $needed_dirs = array()) {
	...
	foreach ( $needed_dirs as $_dir ) {
		...
		if ( ! $wp_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $wp_filesystem->is_dir( $_dir ) )
			return new WP_Error( 'mkdir_failed_pclzip', __( 'Could not create directory. (739)' ), substr( $_dir, strlen( $to ) ) );
	}
	unset($needed_dirs);
		...
		if ( 'f' == $fileinfo['type'] ) {
			...
			if ( !$wp_filesystem->is_dir($to . $filename) ) {
				if ( !$wp_filesystem->mkdir($to . $filename, FS_CHMOD_DIR) )
					return new WP_Error( 'mkdir_failed_copy_dir', __( 'Could not create directory. (790)' ), $to . $filename );
			}
...
function WP_Filesystem( $args = false, $context = false ) {
	...
	if ( ! defined('FS_CHMOD_DIR') )
		// define('FS_CHMOD_DIR', ( substr(sprintf('%o', fileperms(ABSPATH), -4) & 0777 | 0755 ) ));
		define('FS_CHMOD_DIR', 0755 );
	if ( ! defined('FS_CHMOD_FILE') )
		// define('FS_CHMOD_FILE', ( substr(sprintf('%o', fileperms(ABSPATH . 'index.php'), -4 ) & 0777 | 0644 ) ));
