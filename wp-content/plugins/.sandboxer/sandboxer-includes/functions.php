...
function custom_sbox_project() {
	...
	$args = array(
		'label'               => __( 'sbox_project', 'text_domain' ),
	...
	register_post_type( 'sbox_project', $args );

}

// Hook into the 'init' action
add_action( 'init', 'custom_sbox_project', 0 );
...
function sboxr_meta_boxes() {
	global $post;
	add_meta_box( 'sboxr_dimensions', __( 'Project Details', 'sandboxer' ), 'sboxr_meta_box_settings', 'sbox_project', 'side', 'high');
}
...
function sboxr_meta_all_update($post_id) {
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) 
        return;
    if ( 'sbox_project' == @$_POST['post_type'] ) {      
...
<h3 class="hndle"><span>Rate Sandboxer</span></h3>
<div class="inside">
EOD;

$output .= 'You can help us by doing two simple things: <a href="'.$sboxr_options['wordpress_source'].'" target="_blank">Go to WordPress.org now and give this plugin a 5-star rating</a>. Blog about Sandboxer and link to the <a href="'.$sboxr_options['wordpress_source'].'" target="_blank">plugin page</a>. Spreading the word helps us grow.<br/><br/>';
$output .= '<strong>Help Yourself! </strong>Upgrade to Sandboxer Pro! <a href="'.$sboxr_options['home_page'].'">Click here</a> to get the extra features.';
...
function sbox_copy_table($from, $to) {
    if(sbox_table_exists($to)) {
        $success = false;
    }
    else {
...
function sbox_table_exists($tablename, $database = false) {
...
function sboxr_check_license($license) {
	...
	/*
	$store_url = 'http://license.thosedewolfes.com';
	$item_name = 'sandboxer';
