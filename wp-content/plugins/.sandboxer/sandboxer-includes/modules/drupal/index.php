...
function sbox_drupal_make_subsite($prefx, $suffx, $longphrase, $tid) {
	global $settings;
	...
	if ($source = @file_get_contents($settings['baseline'])) {
		...
		// $sbox_drupal_dbprefix = sbox_drupal_dbprefix($prefx,$suffx)."_";
		$tid = intval($tid);

		$from = array('{machine}', '{full}', '{underscore}', '{sbox_drupal_dbprefix}', '{filedir}', '{tid}', '{longphrase}', '{prefx}', '{lat}', '{long}');
		$to = array($machine, $full, $underscore, $sbox_drupal_dbprefix, $filedir, $tid, $longphrase, $prefx, $lat, $long);
		...
		if ($dbconnect) {
			...
			foreach ($db_prefix as $table) {
				if(copy_table('drup_'.$table, $sbox_drupal_dbprefix.$table)) {
					echo "success - $sbox_drupal_dbprefix$table created\n";
				}
				else {
					echo "failure $sbox_drupal_dbprefix$table already exists\n";
				}
			}		
		}
	}
...
function sbox_drupal_dbprefix($prefx, $suffx) {
	$output = "";
