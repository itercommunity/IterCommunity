<?php
		...
		)
	{
		...
		if (isset($mzr_account_type) && isset($mzr_api_user_id))
		{
			...
			{
				$icp_xml = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>\n";
				...
				if (isset($_GET['displayImageByCitationID']) && preg_match("/^[a-zA-Z0-9]+$/", $_GET['displayImageByCitationID']))
					$images = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."icpress_images WHERE citation_id='".trim($_GET['displayImageByCitationID'])."'");
				else
					$images = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."icpress_images");
			...
			else
			{
				...
				// PUBLIC KEY
				$icp_account = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."icpress WHERE api_user_id='".$mzr_api_user_id."'");
				...
				if (isset($_GET['children']) && preg_match("/^[a-zA-Z0-9]+$/", $_GET['children']))
					$icp_url = "https://api.zotero.org/".$mzr_account_type."/".$mzr_api_user_id."/".$urlDataType."/".$_GET['children']."/children?key=".$public_key;
				else if (isset($_GET['topLevel']) && trim($_GET['topLevel']) == "true")
					$icp_url = str_replace("/items", "/items/top", "https://api.zotero.org/".$mzr_account_type."/".$mzr_api_user_id."/".$urlDataType."?key=".$public_key.$content.$style.$order.$sort.$mzr_limit);
				else
					$icp_url = "https://api.zotero.org/".$mzr_account_type."/".$mzr_api_user_id."/".$urlDataType."?key=".$public_key.$content.$style.$order.$sort.$mzr_limit;
				...
				$icp_request = new ICPressRequest();
				$icp_xml = $icp_request->get_request_contents( $icp_url, $mzr_force_recache );
			}
