(function()
{
	...
	var zpTinyMCEWidget = function(ed, url, type, shortcode, summary)
	{
		...
		var zptitle = 'ICPress Bibliography';
		if (type == 'cite') zptitle = 'ICPress In-Text Citation';
		else if (type == 'list') zptitle = 'ICPress In-Text Bibliography';
    ...
    tinymce.create('tinymce.plugins.icpress',
	{  
        init : function(ed, url)
		{
			// Styles
			ed.contentCSS.push( url + "/css/icpress.tinymce.css?" + new Date().getTime() );
			
			// Buttons
			ed.addButton('icpress-cite',
			{
			...
			{
				if ( jQuery(e.target).hasClass('icp-ICPressShortcode') )
				{
			...
			ed.onDblClick.add(function(ed, e)
			{
				...
				if ( jQuery(zpTarget).hasClass('icp-ICPressShortcode') || jQuery(zpTarget).hasClass('summary') )
				{
    ...
    tinymce.PluginManager.add('icpress', tinymce.plugins.icpress);  
}
)();