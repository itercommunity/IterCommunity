<?php
    ...
    class ICPressSidebarWidget extends WP_Widget
    {
        function ICPressSidebarWidget()
        {
            $widget_ops = array('description' => __('Display your citations on your sidebar', 'icp-ICPressSidebarWidget'));
	    parent::WP_Widget(false, __('ICPress Widget'), $widget_ops);
        }
	    ...
	    echo "<div class=\"icp-ICPressSidebarWidget\">\n\n";
	    
	    $icp_sidebar_shortcode = "[icpress";
	    
	    if ($api_user_id)	{ $icp_sidebar_shortcode .= " userid='$api_user_id' "; }
	    if ($nickname)	{ $icp_sidebar_shortcode .= " nickname='$nickname' "; }
	    if ($author)		{ $icp_sidebar_shortcode .= " author='$author' "; }
	    if ($year)		{ $icp_sidebar_shortcode .= " year='$year' "; }
	    if ($data_type)	{ $icp_sidebar_shortcode .= " datatype='$data_type' "; }
	    if ($collection_id)	{ $icp_sidebar_shortcode .= " collection='$collection_id' "; }
	    if ($item_key)	{ $icp_sidebar_shortcode .= " item='$item_key' "; }
	    if ($tag_name)	{ $icp_sidebar_shortcode .= " tag='$tag_name' "; }
	    if ($style)		{ $icp_sidebar_shortcode .= " style='$style' "; }
	    if ($limit)		{ $icp_sidebar_shortcode .= " limit='$limit' "; }
	    if ($sort)		{ $icp_sidebar_shortcode .= " order='$sort' "; }
	    if ($sortby)		{ $icp_sidebar_shortcode .= " sortby='$sortby' "; }
	    if ($image)		{ $icp_sidebar_shortcode .= " showimage='$image' "; }
	    if ($download)	{ $icp_sidebar_shortcode .= " download='$download' "; }
	    if ($title)		{ $icp_sidebar_shortcode .= " title='$title' "; }
	    if ($cite)		{ $icp_sidebar_shortcode .= " cite='$cite' "; }
	    if ($notes)		{ $icp_sidebar_shortcode .= " note='$notes' "; }
	    if ($inclusive)		{ $icp_sidebar_shortcode .= " inclusive='$inclusive' "; }
	    
	    $icp_sidebar_shortcode = trim($icp_sidebar_shortcode) . "]";
	    
	    echo do_shortcode($icp_sidebar_shortcode);
	    
	    echo "</div><!-- .icp-ICPressSidebarWidget -->\n\n";
        ...
        function form( $instance )
        {
            ...
            ?>
                ...
                <style type="text/css">
                <!--
		    #icp-Sidebar-Widget-Container select {
			background-color: #fff;
		    }
                    div.icp-ICPressSidebarWidget-Required span.req {
                        color: #CC0066;
                        font-weight: bold;
                        font-size: 1.4em;
                        vertical-align: -35%;
                    ...
                    div.icp-ICPressSidebarWidget-Required {
		...
		<div id="icp-Sidebar-Widget-Container">
		    ...
		    <div class="icp-ICPressSidebarWidget-Required">
		...
		</div> <!-- #icp-Sidebar-Widget-Container -->
                
            <?php
        }
    }
    
    function ICPressSidebarWidgetInit() {
        register_widget( 'ICPressSidebarWidget' );
    }
