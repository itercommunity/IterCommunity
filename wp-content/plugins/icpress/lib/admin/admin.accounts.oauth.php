...
/** Code provided by and adapted from: http://www.zotero.org/support/dev/server_api/v2/oauth
   ...
   **/
    ...
    $callbackUrl = $_GET['return_uri'] . '/wp-content/plugins/icpress/lib/admin/admin.accounts.oauth.php?oauth_user='.$_GET['oauth_user'].'&return_uri='.$_GET['return_uri'];
    ...
    function read_state(){
		global $wpdb;
		$oa_cache = $wpdb->get_results("SELECT cache FROM ".$wpdb->prefix."icpress_oauth");
		return unserialize( $oa_cache[0]->cache );
    }
    function write_state($state)
    {
		global $wpdb;
		$oa_cache = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."icpress_oauth");
		$query = "UPDATE ".$wpdb->prefix."icpress_oauth ";
  ...
  switch($state['oauthState'])
  {
    case 0:
        ...
        $redirect = '
                <script type="text/javascript" src="'.$_GET['return_uri'].'/wp-content/plugins/icpress/js/jquery-1.5.2.min.js"></script>
                <script type="text/javascript">
                
                jQuery(document).ready(function()
                {
                    jQuery("body").addClass("icp-Modal");
                });
    ...
    {
        // ADD PRIVATE KEY TO THE USER'S ACCOUNT IN ICPRESS
        global $wpdb;
        $query = "UPDATE ".$wpdb->prefix."icpress ";
