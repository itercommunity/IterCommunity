...
function sboxr_wordpress_make_subsite($args = array()) {
	global $settings;
	...
	if ($source = @file_get_contents($settings['baseline'])) {
		...
		$filedir = str_replace(" ","_",sboxr_defang(strtolower($prefx." ".$suffx)));
		// $sboxr_wordpress_dbprefix = sboxr_wordpress_dbprefix($prefx,$suffx)."_";
		$tid = intval($tid);

		$from = array('{machine}', '{full}', '{underscore}', '{sboxr_wordpress_dbprefix}', '{filedir}', '{tid}', '{longphrase}', '{prefx}', '{lat}', '{long}');
		$to = array($machine, $full, $underscore, $sboxr_wordpress_dbprefix, $filedir, $tid, $longphrase, $prefx, $lat, $long);
		...
		if ($dbconnect) {
			...
			foreach ($db_prefix as $table) {
				if(copy_table('drup_'.$table, $sboxr_wordpress_dbprefix.$table)) {
					echo "success - $sboxr_wordpress_dbprefix$table created\n";
				}
				else {
					echo "failure $sboxr_wordpress_dbprefix$table already exists\n";
				}
			}		
		}
	}
...
function sboxr_wordpress_dbprefix($prefx, $suffx) {
	$output = "";
